...
//var currentVolume = 0.3;  // 初期値として定義しておく
...
//	}
//	else {
		soundfile[n] = new Audio(filename);       // ← Audio オブジェクトを生成
		soundfile[n].volume = 0.3;
		sf_name[n] = filename;
//	}
	soundloaded++;
}
...
function playSE(n) {
	...
	if(wait_se == 0) {
		seNo = n;
		//soundfile[n].volume = 0.2;                // ← 音量を下げる（0.0 ～ 1.0）
...
function playBgm(n) {
	...
	log("ＢＧＭ"+n+"出力");
	bgmNo = n;
	//soundfile[n].volume = 0.1;                // ← 音量を下げる（0.0 ～ 1.0）
